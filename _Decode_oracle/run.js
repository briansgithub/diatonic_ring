 ...
 * run the engine, compare, and write reports under _Decode_oracle/out/<slug>/.
 *
 * Usage:
 *   node _Decode_oracle/run.js <url> [<url> ...]
 *   node _Decode_oracle/run.js --corpus _Decode_oracle/corpus.json
 *   node _Decode_oracle/run.js --corpus _Decode_oracle/corpus.json --rescrape
 *
...
function mergeMatrix(into, from) {
  for (const [k, e] of from) {
    if (!into.has(k)) into.set(k, { total: 0, romanExact: 0, romanCore: 0, notesOk: 0 });
    const t = into.get(k);
    t.total += e.total; t.romanExact += e.romanExact; t.romanCore += e.romanCore; t.notesOk += e.notesOk;
  }
}

function writeGlobal(results, globalMatrix, globalDiscrepancies) {
  ...
  }), { total: 0, romanExact: 0, romanCore: 0, notesOk: 0 });
  ...
  sm += `- Notes consistent: **${pct(agg.notesOk, agg.total)}** (${agg.notesOk}/${agg.total})\n\n`;
  sm += `| Song | chords | romanExact | romanCore | notesOk | disc |\n|---|---|---|---|---|---|\n`;
  results.forEach((r) => { sm += `| ${r.slug} | ${r.total} | ${pct(r.romanExact, r.total)} | ${pct(r.romanCore, r.total)} | ${pct(r.notesOk, r.total)} | ${r.discrepancies} |\n`; });
  ...
  [...globalMatrix.entries()].sort().forEach(([k, e]) => {
    am += `| ${k} | ${e.total} | ${pct(e.romanExact, e.total)} | ${pct(e.romanCore, e.total)} | ${pct(e.notesOk, e.total)} |\n`;
  });
...
async function runResolvedSong(resolved) {
  const cmp = await compareSong(resolved.scrape);
  const rep = buildReport(cmp, resolved.scrape, resolved.dir);
  console.log(`[${resolved.slug}] chords=${rep.total} romanExact=${rep.romanExact} romanCore=${rep.romanCore} notesOk=${rep.notesOk} disc=${rep.discrepancies}`);
  return { result: { slug: resolved.slug, url: resolved.url, total: rep.total, romanExact: rep.romanExact, romanCore: rep.romanCore, notesOk: rep.notesOk, discrepancies: rep.discrepancies }, rep };
}
...
async function runCorpus(corpusFile, opts) {
  ...
  for (const entry of entries) {
    ...
    const { result, rep } = await runResolvedSong(resolved);
  ...
  const agg = writeGlobal(results, globalMatrix, globalDiscrepancies);

  console.log('\n===== CORPUS AGGREGATE =====');
  results.forEach((r) => console.log(`  T${r.tier} ${r.slug.padEnd(42)} chords=${String(r.total).padStart(4)} romanExact=${pct(r.romanExact, r.total).padStart(4)} notesOk=${pct(r.notesOk, r.total).padStart(4)} disc=${r.discrepancies}`));
  console.log(`  TOTAL: ${results.length} songs, ${agg.total} chords | romanExact ${pct(agg.romanExact, agg.total)} | romanCore ${pct(agg.romanCore, agg.total)} | notesOk ${pct(agg.notesOk, agg.total)}`);
...
async function runOne(url, opts) {
  ...
  const { result } = await runResolvedSong(r);
  return result;
}
...
async function main() {
  ...
  const agg = results.reduce((a, r) => ({ total: a.total + r.total, romanExact: a.romanExact + r.romanExact, romanCore: a.romanCore + r.romanCore, notesOk: a.notesOk + r.notesOk }), { total: 0, romanExact: 0, romanCore: 0, notesOk: 0 });
  console.log('\n===== AGGREGATE =====');
  results.forEach((r) => console.log(`  ${r.slug.padEnd(40)} chords=${String(r.total).padStart(4)} romanExact=${String(r.romanExact).padStart(4)} notesOk=${String(r.notesOk).padStart(4)} disc=${r.discrepancies}`));
  console.log(`  TOTAL chords=${agg.total} romanExact=${agg.romanExact} romanCore=${agg.romanCore} notesOk=${agg.notesOk}`);
}

if (require.main === module) main();

module.exports = { runOne, slugForUrl };
