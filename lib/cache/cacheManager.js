...
const { getPlaybackCacheDir } = require('../dataRoot');

const CACHE_DIR = getPlaybackCacheDir();
...
function saveSongMetadata(url, songTitle, metadata) {
  try {
    const { artist } = extractArtistAndSongFromUrl(url);
    const songDir = getSongCacheDir(artist, songTitle);
    ...
    console.log('✓ Saved song metadata');
  } catch (e) {
    console.log('✗ Metadata save error:', e.message);
  }
}

function loadSongMetadata(artist, songTitle) {
  try {
    const songDir = getSongCacheDir(artist, songTitle);
    const metadataPath = path.join(songDir, '_metadata.json');
    ...
    console.log('✗ Metadata load error:', e.message);
    return null;
  }
}
