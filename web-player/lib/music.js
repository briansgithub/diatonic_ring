...
function getChordQualityFromCustomScale(borrowedArray, degree) {
  ...
  // Intervals are absolute pcs from tonic; normalize third/fifth above root pitch.
  ...
  if (isMajorThird && isPerfectFifth) {
    ...
    return "diminished";
  } else if (isMajorThird && fifthSemitones === 8) {
    return "augmented";
  } else {
...
export function rootToDiatonicTriad(chordRootSD, key, baseOctave, borrowed = null, chordType = 5, inversion = 0, applied = 0, suspensions = [], modifierChord = null) {
  ...
  // Holst Mars II△42 only — raised root on native phdm-key 3rd inversion maj7.
  ...
  if (phdmIImaj7) {
    chordRootNoteName = shiftPitchClass(chordRootNoteName, 1);
  }
